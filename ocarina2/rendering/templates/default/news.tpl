		...
		{else}
			...
			{foreach from=$commenti key=key item=item}
				{if $commenti[$key]->approvato == 1}
					<fieldset><legend><a href="commento.php?id={$commenti[$key]->id}">#{$item@iteration}</a> Commento inviato il giorno {$commenti[$key]->data} alle ore {$commenti[$key]->ora} da <a href="profilo.php?nickname={$commenti[$key]->autore}">{$commenti[$key]->autore}</a>. {if ((isset($grado)) && (is_numeric($grado)) && ($grado < 3))}<a href="admin/cancellacommento.php?id={$commenti[$key]->id}">(X)</a>{/if}</legend><div onclick="javascript:quota(this);">{$commenti[$key]->contenuto}</div></fieldset><br />
				{/if}
			{/foreach}
		{/if}
		<br />
