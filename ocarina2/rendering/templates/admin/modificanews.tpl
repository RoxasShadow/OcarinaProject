{include file="$root_rendering/templates/$skin/include/header.tpl"}
	...
	{elseif $grado < 4 && !$submit && !isset($result)}
		...
		Titolo<br />
		<input type="text" name="titolo" value="{$titolo_news}" /><br /><br />
		Categoria<br />
		<select name="categoria">
		{foreach from=$categorie key=key item=item}
			{if $categorie[$key] == $categoria_news}
		...
		{/if}
		<textarea name="news" cols="59" rows="10" id="txtQuota">{$testo_news}</textarea><br />
		<input type="hidden" name="sel_news" value="{$sel_news}" />
