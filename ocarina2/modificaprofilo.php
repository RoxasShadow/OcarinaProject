...
if($logged)
	if(!$submit) {
		...
		if(($user->isEmail($email)) && ($user->isImage($avatar)) && ($email !== '') && ($bio !== '') && ($avatar !== '') && ($password !== ''))
			...
			elseif(($user->editUser('email', $email, $username[0]->nickname)) && ($user->editUser('bio', $bio, $username[0]->nickname)) && ($user->editUser('avatar', $avatar, $username[0]->nickname)))
...
$rendering->renderize('modificaprofilo.tpl');
