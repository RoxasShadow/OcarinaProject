...
$rendering->addValue('grado', $pagina->isLogged() ? $user->username[0]->grado : '');
$rendering->skin = 'admin';
$rendering->addValue('titolo', 'Cancella pagina &raquo; Amministrazione &raquo; '.$pagina->config[0]->nomesito);

if(($pagina->isLogged()) && ($user->username[0]->grado < 3))
	if(!$submit)
		...
		if($minititolo_pagina !== '')
			if($pagina->deletePage($minititolo_pagina)) {
				...
				if($pagina->config[0]->log == 1)
					$pagina->log($user->username[0]->nickname, 'Page \''.$minititolo_pagina.'\' deleted.');
			}
			else {
				$rendering->addValue('result', 'È accaduto un errore durante la cancellazione della pagina.');
				if($pagina->config[0]->log == 1)
					$pagina->log($user->username[0]->nickname, 'Page \''.$minititolo_pagina.'\' deletion failed.');
			}
		else {
...
(($pagina->isLogged()) && ($user->username[0]->grado == 7)) ? $rendering->renderize('bannato.tpl') : $rendering->renderize('deletecontent.tpl');
