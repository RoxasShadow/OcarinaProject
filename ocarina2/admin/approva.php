		...
		if($username[0]->grado < 3)
			if($minititolo_news !== '')
				if($comments->editNews('approvato', '1', $minititolo_news)) {
					if($comments->config[0]->log == 1)
						$comments->log($username[0]->nickname, 'News '.$minititolo_news.' approved.');
					$rendering->addValue('result', 'La news è stata approvata ed è ora visibile.');
				}
				else {
					if($comments->config[0]->log == 1)
						$comments->log($username[0]->nickname, 'News '.$minititolo_news.' approvation failed.');
			...
			elseif($id_commento !== '')
				if($comments->editComment('approvato', '1', $id_commento)) {
					if($comments->config[0]->log == 1)
						$comments->log($username[0]->nickname, 'Comment #'.$id_commento.' approved.');
					$rendering->addValue('result', 'Il commento è stato approvato ed è ora visibile.');
				}
				else {
					if($comments->config[0]->log == 1)
						$comments->log($username[0]->nickname, 'Comment #'.$id_commento.' approvation failed.');
			...
			elseif($minititolo_pagina !== '')
				if($page->editPage('approvato', '1', $minititolo_pagina)) {
					if($comments->config[0]->log == 1)
						$comments->log($username[0]->nickname, 'Page '.$minititolo_pagina.' approved.');
					$rendering->addValue('result', 'La pagina è stata approvata ed è ora visibile.');
				}
				else {
					if($comment->config[0]->log == 1)
						$comment->log($username[0]->nickname, 'Page '.$minititolo_pagina.' approvation failed.');
...
(($logged) && ($username[0]->grado == 7)) ? $rendering->renderize('bannato.tpl') : $rendering->renderize('approva.tpl');
